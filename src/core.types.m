...
%
% We use tagged pointers to implement discrimitated unions.  Most of what we
...
% constants with the higher bits used to encode each constant.  The the
% next remaining primary tags "01" "10" are given to the first two
% constructors with arguments and the final tag "11" is used for all the
% remaining constructors, utilizing a second tag as the first field in the
% pointed-to structure to distinguish between them.
%
% This scheme has a specific benifit that is if there is only a single
...
type_setup_ctor_tags(!Type) :-
    ...
    Ctors = !.Type ^ t_ctors,
    foldl2(make_enum_tag_info(0), Ctors, 0, _, init, TagInfos),
