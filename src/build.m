...
write_ninja_rules_file(Verbose, Result, !IO) :-
    write_file(Verbose, ninja_rules_file,
...
rule plztyperes
    command = $path/plzc --mode make-typeres-exports $in -o $out
    description = Calculating type & resource exports for $name

rule plzidep
    command = $path/plzc --mode make-interface-depends --target-file $target $in -o $out
...
rule plzlink
    command = $path/plzlnk -n $name -o $out $in
